...
if(count($data2)<=0){
    ...
    <div class="main-box-body clearfix">
            <table class="Ying table user-list table-hover">
                ...
                <tr >
                    ...
                    <th class="col-xs-2 col-sm-2 ">Thời gian</th>
    ...
    for($i = 0; $i<count($data2); $i++) {
        ...
        $dbdateEnd = $data2[$i]->enddate;
        $dbdateStart = $data2[$i]->startdate;
        $myDateEnd = substr($dbdateEnd,8,2).'/'.substr($dbdateEnd,5,2).'/'.substr($dbdateEnd,0,4);
        $myDateStart = substr($dbdateStart,8,2).'/'.substr($dbdateStart,5,2).'/'.substr($dbdateStart,0,4);
        $myString = " - ";
        echo '<td>'.$myDateStart.$myString.$myDateEnd.'</td>';
        echo '<td >';
            if ($data2[$i]->status == 1) {
